...
### Can all formal languages be encoded as regular expressions?
Formal languages consists of words whose letters are taken from the alphabet and are formed according to a specific set of rules. In computer science, formal languages are used as a basis for defining the grammer of programming languages. This is the basis of the Turing machine, invented by Alan Turing in 1936. A turing machine is a hypothetical machine that can simulate any computer algorithm, no matter how complicated it is. A turing machine is non-deterministic as it can simulate any computer algorithm, for every language that can be recognized by a turing machine there is an infinite number of turing machines that can recognize the same language. Because of this I dont think there is a defined answer to this question, however in my own research and opionion; I believe that regular expressions are not turing complete. 

Turing completeness is when a system or a language can compute every Turing-computable function and since regular expressions can be turned into deterministic finite automatons. Which is a finite-state machine that accepts or rejects a given string of text, which will have a start and end state. Since a turing machine is non deterministic and can travel back and fort between states and a deterministic regular expression cannot access the start state once it reaches the end state, I believe not all formal languages can be encoded as regular expressions.
